...
A local hospital doing cancer gene research had the issue of manually going through Clinvar's database to find specific genes and formating it into a table. Our team project was to create a website for them to enter gene information that would generate the formated table to save them time. Our project would first download the files from Clinvar's webiste on a weekly basis. Then filter the data for only certain genes and place the data into a database. Depending on the input from the website it would filter through the database finding all releavent genes. Finally formating them into a table for a specified file type selected by the user. I handled setting up the database, automating the file downloads from Clinvar and placing the filtered data into the database. While the frontend developer handled the design of the pages, the filter that takes in the input from website to get relevant data. We both worked on integrating how the frontend and backend communicated. The techincal writer handled the documentaion of the project. While the team lead created the schedule and attempted to host the project but could not find a free option due to the large amount of data needed to be stored. 
